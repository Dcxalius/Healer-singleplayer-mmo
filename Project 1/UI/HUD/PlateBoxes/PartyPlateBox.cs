...
namespace Project_1.UI.HUD.PlateBoxes
{
    internal class PartyPlateBox : PlateBox
    {
        ...
        public PartyPlateBox(RelativeScreenPosition aPos, RelativeScreenPosition aSize, int aIndex) : base(aPos, aSize)
        {
            ...
            border = new CommandBorder(this, Color.YellowGreen, RelativeScreenPosition.Zero, aSize);
            VisibleBorder = false;
            Visible = false;
        }
        ...
        public override void ClickedOnAndReleasedOnMe()
        {
            ...
            MailboxManager.PublishSimCommand(new TargetRequested(guildMemberRenderId));
        }
        ...
        protected override void ClickedOnMe(ClickEvent aClick)
        {
            ...
            {
                MailboxManager.PublishSimCommand(new PartyCommandRequested(PartyCommandAction.Add, guildMemberRenderId));
            }
            else if (aClick.Modifier(InputManager.HoldModifier.Ctrl))
            {
                MailboxManager.PublishSimCommand(new PartyCommandRequested(PartyCommandAction.NeedyAdd, guildMemberRenderId));
            }
